...
public class MTEHatchRealityPhaseSensorGui extends MTEHatchBaseGui<MTERealityPhaseSensor> {

    public MTEHatchRealityPhaseSensorGui(MTERealityPhaseSensor hatch) {
        super(hatch);
    }
    ...
    public Flow createThresholdFieldRow() {
        return Flow.row()
            .child(
                new TextFieldWidget().setNumbers(0, 1000)
                    .size(77, 12)
                    .value(new DoubleSyncValue(hatch::getThreshold, hatch::setThreshold))
                    .setFocusOnGuiOpen(true))
            .child(
                IKey.lang("GT5U.gui.text.phase_sensor")
                    .asWidget())
            .coverChildren()
            .childPadding(2);
    }
